...
public interface IntegrationSearchRepository {
    ...
    @Query(value = "SELECT new com.momentum.dto.response.community.IntegrationCommunitySearchDto("
            + "community.id, community.title, community.created_at, community.likes, community.hits) " +
            "FROM (" +
            "SELECT cp.id, p.title, p.content, p.created_at, p.likes, p.hits " +
            "FROM ConcernPost cp " +
            "JOIN Post p ON cp.post_id = p.id " +
            "UNION ALL " +
            "SELECT srp.id, p.title, p.content, p.created_at, p.likes, p.hits " +
            "FROM SurgeryReviewPost srp " +
            "JOIN Post p ON srp.post_id = p.id " +
            "UNION ALL " +
            "SELECT dp.id, p.title, p.content, p.created_at, p.likes, p.hits " +
            "FROM DailyPost dp " +
